...
def dashboard_view(request):
    ...
    context = {
        'flight_orders': Payment.objects.filter(order__user__user = request.user).order_by('-id'),
        'tour_orders': TourPaymentsModel.objects.filter(order__user = request.user).order_by('-id')
    }
